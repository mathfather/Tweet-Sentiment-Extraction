...
from models import BertBaseForQAWithoutSentiment
from functions import TrainWithOutSentiment, EvaluateWithOutSentiment, log_tracker
######## Hyperparameter ########
batch_size = 32
epochs = 10
lr = 2e-5
eps=1e-8
warmup_steps = 0
model_name = config.BERTBASEWITHOUTSENTIMENT
split_ratio = 0.10
random_seed = 233
######## Hyperparameter ########
...
model = BertBaseForQAWithoutSentiment()
model.to(device)
...
for epoch in range(epochs):
    ...
    jaccard = EvaluateWithOutSentiment(dev_loader, model, device, selection_method="maximum")
